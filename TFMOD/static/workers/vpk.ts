...
function offsetOutOfBounds(offset: number, treeSize: number, fromMethod = "", skipCount: number, initialOffset: number) {
	console.log(offset, treeSize + initialOffset);
	if (offset > treeSize + initialOffset) {
		console.error("Exceeded tree size");
	}
	if (offset - skipCount > treeSize + initialOffset) throw "Something went wrong, offset out of bounds. Was in: " + fromMethod;
}
...
export function read(files: Array<Uint8Array>): Array<Entry> {
	...
	const initalOffset = version === 2 ? 0x1C : 0x0C;
	...
	while (offset < treeSize) {
		...
		if (extensionSkip) {
			...
			while (dataview.getUint8(offset) !== 0) {
				...
				offsetOutOfBounds(offset, treeSize, "Extension", skipCount, initalOffset);
				offset++;
			}
		}
		offsetOutOfBounds(offset, treeSize, "After Extension", skipCount, initalOffset);
		offset++;
		let path = "";
		if (pathSkip) {
			path = pathSkip;
			...
			while (dataview.getUint8(offset) !== 0) {
				...
				offsetOutOfBounds(offset, treeSize, "Path", skipCount, initalOffset);
				offset++;
			}
		}
		offsetOutOfBounds(offset, treeSize, "After Path", skipCount, initalOffset);
		offset++;
		let name = "";
		while (dataview.getUint8(offset) !== 0) {
			...
			offsetOutOfBounds(offset, treeSize, "Name", skipCount, initalOffset);
			offset++;
		}
		offsetOutOfBounds(offset, treeSize, "After Name", skipCount, initalOffset);
		offset++;
		console.info(path + "/" + name + "." + extension);
		const crc = dec(dataview, offset);
		offsetOutOfBounds(offset, treeSize, "After CRC", skipCount, initalOffset);
		offset += 4;
		const preload = decimal(short(dataview, offset));
		offsetOutOfBounds(offset, treeSize, "After preload", skipCount, initalOffset);
		offset += 2;
		...
		offsetOutOfBounds(offset, treeSize, "After Archive Index", skipCount, initalOffset);
		offset += 2;
		...
		offsetOutOfBounds(offset, treeSize, "After Entry Offset", skipCount, initalOffset);
		offset += 4;
		const entryLength = dec(dataview, offset);
		offsetOutOfBounds(offset, treeSize, "After Entry Length", skipCount, initalOffset);
		offset += 4;
		...
		offsetOutOfBounds(offset, treeSize, "After Terminator", skipCount, initalOffset);
		offset += 2;
		if (decimal(short(dataview, offset)) === 0) {
			extensionSkip = false;
			pathSkip = false;
			skipCount += 2;
			offsetOutOfBounds(offset, treeSize, "No Skip", skipCount, initalOffset);
			offset += 2;
			...
			extensionSkip = extension;
			skipCount++;
			offsetOutOfBounds(offset, treeSize, "Extension Skip", skipCount, initalOffset);
			offset++;
		}
